...
> ⚠ **Corrected the same day — see §7 and `NOTE-running-step-1-on-the-condensate.md`.** The *scope*
> half of this subsection is wrong. On the axioms as written the condensate is **not** out of scope:
> A2.1 makes every vantage a stream, naming a rock and an electron, and A3.5 gives every stream a γ.
> §9.6's exclusion is a gloss that the axiom tier does not support. The *gate was never run* half
> stands, and has since been run. **The §4 retraction is unaffected: it rests on §4a and §4b, neither
> of which depends on scope.**
...
## 7. Addendum, same day — §10 Step 1 was run, and §4c's scope argument does not survive it

*Added 2026-09-13, a few hours after this note was committed as `0208d0b`. Full run in
`NOTE-running-step-1-on-the-condensate.md`; this is the part that corrects what is above.*

§6's Owed list put "run §10 Step 1 on the condensate" first. It has been run. Three consequences for
this note:

**1. §4c is half wrong, and an inline warning has been placed at its heading.** I wrote that the
condensate test was *out of scope*. It was not. **A2.1** states *"Every vantage within X is a
stream"* and its prose names the examples explicitly — *"A rock has a vantage; an electron has a
vantage"* — and refuses to draw a line on the grounds that *"there is no principled way"* to draw
one. **A3.5** gives every stream a γ. **A1** cannot fail, because σ is localization in a Ground that
is everything. So the gate admits the condensate, and §9.6's exclusion of *"rocks-alone,
atoms-alone"* is a gloss that names the same two objects A2.1 admits and reaches the opposite
verdict. §4c read §9.6 as if it bound the axioms. It does not; it conflicts with them, which is a
different and more interesting fact.

**The §4 retraction does not move.** It stands on §4a (C_sep's `DOF(O₁), DOF(O₂)` ranges over
*objectives*, so the predicate is undefined on a condensate) and §4b (N is pairs crossing per
oscillation; ξ = 1/√N is counting statistics with likelihood ratio 1). Neither mentions scope.

**2. "C_scale VACUOUS — one collective scale" was also wrong**, and for a reason available in my own
repository. The DAG's nodes name nested scales for this very object: `em.scalar_mode_london_mass`
carries a London depth of 22.5 nm against a barrier of ~39 nm and a ratio ω_L/ω from 4.2 × 10⁵ to
5.0 × 10⁴; `em.transmon_bounds_scalar_channel` carries E_J/E_C and a junction inside a cavity inside
a package. A condensate is *not* single-scale. That cell was retracted with the rest of §4, but it
deserves naming separately, because unlike the other three it failed against evidence I already had
rather than against a definition I had misread.

**3. The gate turned out to be the finding.** §10 Step 1 **cannot fail** — no object with a location
is sent back by it — which makes it a positive control that cannot fail at the head of a seven-step
procedure. But running it componentwise located a real obstruction one axiom further in: **A3.1 and
A3.4 say γ_S is updated as the stream navigates and is "not a fixed transformation," and a Josephson
junction at plasma resonance is generated by a time-independent Hamiltonian.** A3.4-with-teeth and
A3.5-universality cannot both hold, and the electron A2.1 names is the same witness.

The repair, which is the part worth carrying: **A3.4 should be read as history-dependence — the
generator a functional of the trajectory, not of the instantaneous state — which is
non-Markovianity, and is measurable.** Under that reading the DAG can decide it, and the DAG's own
nodes are written on the Markovian side: `em.transmon_bounds_scalar_channel` treats T₁ as a **rate**
cap, and a relaxation time with no memory kernel is the Markovian parametrization.

**That is the first joint running from the DAG to the book** rather than the other way: not the
physics obstructing a metaphysical claim, but the physics supplying a criterion the metaphysics is
missing. §5's live joint was the metaphysics posing a question the DAG could not answer. This is the
reverse, and it is the direction the standing framing question did not anticipate.

**Still owed, and now owed twice:** a measured non-Markovianity for a transmon or a Josephson
junction. The arXiv search tool returned empty on every query in both sessions today, and a citation
from memory is a bare constant wearing a label, so none is offered.
...
**Amended the same day (§7).** §4c's scope argument was itself wrong, and wrong in the direction this
note spends its length warning about: I applied §9.6's gloss as though it constrained the axioms,
without reading A2.1's own prose, which names a rock and an electron as streams. The check that
