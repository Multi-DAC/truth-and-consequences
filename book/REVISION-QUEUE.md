...
| **R-51** | *(Day 189, ruling 160)* | `tools/brief_fields.py` | **NO GAUGE READS A BRIEF FIELD FOR *CONTENT*.** `brief_fields.py` finds holes; **the false `Source:` line that produced ruling 160 passes it clean.** ★ Same shape as R-47 and R-49, and **demonstrably the next one, since the repair for R-49 shipped with the defect R-51 names.** | **Before Book VI drafts** — VI's eight briefs are 4–12 lines with no `Source` and no `Named`, so a hole-finder will pass them and a content-reader would not. | medium |
