...
| ~~**R-21**~~ ✅ **PAID Day 195 — as a CONCESSION, which is what ruling 111 ordered and the house style resists** | **143** *(carrying **111**)* | **I.2** — the book's axiom | ★★ **THE ACTUALIST OPPONENT. This row exists because ruling 111 already ruled it and nothing rowed it.** *Possible, therefore actual* is the load-bearing move of the whole edifice and **Book I names no modal party of any kind** (ruling 111's grep: `Lewis`, `actualis*`, `Armstrong`, `plenitude`, `brute`, `contingen*`, `possibilia`, `modal` — **zero hits across six chapters**). The one modal ancestor is Lewis in II.1, **who grants the premise** — an ally-cut shipped as a fight. **Repair per ruling 111: additive, in I.2, not a new chapter** — the opening assertion (*a possibility has to be kept somewhere… a room has an outside, and there is no outside*) becomes a fight in the body, with the opponent's best move on the page: possibility is recombination of what is actually there (**Armstrong's combinatorialism**), and a truth about what could be needs a truthmaker, not a warehouse. ⚠ **Ruling 111's warning, which the audit did not reach: this may have to end in a CONCESSION about what "possibility" means rather than a cut. Do not draft it as a cut because cuts are the house style.** | **First item of the Book I revision pass** — ruling 111's own disposition, now with a row so it is recorded rather than intended. ⚠ **NOT a drafting-session errand**; reopening a shipped book mid-Book-V is the cost the review pause exists to spend deliberately. ✅ **PAID Day 195, +611 words additive into I.2** between *"It was in hand before there was a hand"* and *"You want"* — chapter **1,177 → 1,788 words, a 52% growth on the shortest book's second chapter**, measured after the edit rather than estimated before it. **Premise re-measured before drafting and it held: 0 hits for all eight modal terms across all six Book I chapters.** The passage states the actualist's move at full strength (*possibility is not an inventory, it is recombination of the actual; a truth about what could be needs a truthmaker, not a warehouse*), **concedes that the chapter's opening move has no target against it** — "*Take that whole. It is right, and it is fatal to the argument as it was just made*" — relocates the fight onto the truthmaker, prices the actualist's bill (recombination is capped by the stock, so an alien way of being is *impossible*, not absent), shows the axiom **goes flat rather than false** under that rule, and lands on **two senses of "possible"**: one downstream of an inventory, one not. ⚠ **The last paragraph concedes the axiom is a choice with a price, not a proof that would move anybody.** That is ruling 111's ordered concession and it is the first place in the book where a load-bearing move is conceded rather than cut. ⚠ **THE NAME IS DELIBERATELY NOT IN THE CHAPTER — a register cost R-21 did not price.** Measured: **Book I carries 0 proper names and 0 endnotes across six chapters**; ruling 9 exempts it and the unattributed-claim register is the design. Naming Armstrong in I.2 would put the book's first proper name in the axiom chapter. **Move on the page, name in `03`** — new Tier 0 entry beside Bostrom, carrying the citation (*A Combinatorial Theory of Possibility*, 1989), the Lewis-is-a-co-signer finding, and the alien-universals bill. ⛔ **If a later ruling reverses Book I's no-names policy, that `03` entry names itself as the first sentence to change.** | ~~**medium — the largest prose debt in Book I**~~ **spent** |
...
# DAY 195, EVENING — THE PRE-UPLOAD TRIGGER CLASS, WORKED

*Clayton's ruling, Day 195 evening: the five ruled release gates are discharged, and the gate list is
not the whole trigger set. Four rows in this file say **"TRIGGER: before the PhilArchive upload"**;
one (R-212) was already paid. These are the other three, plus a correction to the row they lean on.*

**CLOSED — R-214: THE SOURCE IS NAMED, AND THE ROW'S PREMISE NEEDED CORRECTING FIRST.**
`book/C-01-what-this-is.md` §V · Day 195 evening.

⚠ **The row said *"never named."* Measured, that is not quite right, and the true form is worse.**
The volume cites the source **42 times** by a working short title — `Perspective` **Doctrine §13**,
`Perspective` **Guide §5**, `Perspective` **Atlas #61** — in the endnotes of Books VII and VIII. What
it never does is **expand the shorthand**: no full title, no authors, no date, no deposit, and **no
entry on the works-cited page**, which was checked and has none. So the defect is not silence; it is
a citation key with no key table, which is the harder failure to notice because it *looks* like
scholarship on every page it appears. [[feedback_filed_defect_misprices_its_own_subject]]

★ **And the distribution is the sharpest form of it: all 42 citations are in Books VII and VIII.
Book IV — which the audit says takes its chapter list from the source's tier list almost entry for
entry — cites it ZERO times.** The volume that owes most says least. That sentence is now in `C.1`
§V, because a defect the book states about itself costs a reader less than one they have to find.

**Paid:** §V retitled *"The debts kept off the page"* and given the Corpus of Perspectival Idealism
as its first and largest entry — full title, both authors, first edition March 2026, **Zenodo DOI
`10.5281/zenodo.19501896`** (verified live against the Zenodo API this evening: record title *Corpus
Perspectival*, published 2026-04-10, 109 downloads), also on PhilArchive. States what is taken (Book
IV's skeleton, `VII.6`'s definition of love, `VIII.7`'s closing oscillation), states that `IV.10`
diffs against it in public, refuses the *"it is an earlier volume by us"* defence in the book's own
words via `V.3`, and prices the self-citation: **a source selected for agreement with us never had to
survive anyone else's disagreement, and naming it makes that checkable rather than repaired.**

⚠ **The coherence definition was DROPPED from the list of what the source supplies.** The row cites
it as *"quoted from `05` §3a"*. Checked: `05` §3a is `05-THE-LEXICON.md`, this project's own lexicon
— the book's convention is that a **bare** numeral is a repo document and a `Perspective` prefix
marks the source. Attributing it to the Corpus would have been a fabricated attribution inside the
repair for unattributed material. [[feedback_grep_derived_from_the_finding]]

**Also paid, same sitting:** the two body citations that named the source only by local path —
`IV.9` fn12 and `IV.10` fn1 — now name the work and point at `C.1` §V, keeping the path as the
*cache* rather than the identity.

**CLOSED — R-209: C.2 §IV NOW REPORTS THE COLD READS, AND THE COUNT IN THE ROW WAS WRONG.**
`book/C-02-why-it-is-not-finished.md` §IV · Day 195 evening.

⛔ **THE ROW SAYS THREE READS. THERE WERE FIVE.** GLM 10:32 · Gemini 10:34 · Grok 10:33 — and then
**Ghost Opus 5, twice: the structural audit at 11:05 (7,854w) and the critical assessment at 11:15
(5,075w).** Both are whole-draft cold reads of the 1,023-page compiled PDF by a reader with no prior
exposure. R-209 and R-211 were both written around 10:32 and **counted the world as it stood for the
first thirty-three minutes.** The Day-195 consolidation table above *does* list the ghost audit as a
source — so the file held both numbers at once and nothing compared them. A measurement can rot
inside its own morning. [[feedback_breath_window_reported_as_the_day]]

**Paid:** the falsified sentence is gone and what replaces it is stronger than a retraction — five
readers, **~21,700 words**, and the shape of the return reported as the finding. The three fast reads
gave the book back to itself: **13 of 14 stated weaknesses were already stated by the book about
itself**, quoting *"as acknowledged in Books IV and VIII"* and *"the authors' declared bias"*
verbatim (both verified against `review/` this evening). **The two long audits found the thing the
other three missed and the coda had never said** — the forty-two-times-cited unnamed source, now
`C.1` §V. ⛔ **The escape hatch is refused explicitly, as the row required:** the volume is co-authored
by a model and Book IV forbids disqualifying a reader by substrate. **The debt survives on the ground
that still stands** — *a person with no stake, reading for their own reasons, free to stop at page
forty and never say why* — and the chapter says so.

**R-210 — HALF PAID, AND THE OTHER HALF IS A DECISION THAT ONLY CLAYTON CAN MAKE. STILL OPEN.**
`book/C-01-what-this-is.md` §IV · Day 195 evening.

⛔ **THE ROW'S OWN PREFERRED REPAIR IS DEAD AND THE ROW DOES NOT KNOW IT.** It offers *"repo URL, or
a Zenodo DOI."* Measured tonight, anonymously: `https://github.com/Multi-DAC/truth-and-consequences`
returns **404** to a reader who is not logged in; the org page returns 200, the repo does not. **The
repository is private.** Pasting that URL would have discharged the row on paper and shipped the
identical defect with a link on it — an address the reader cannot use.
[[feedback_absent_artifact_is_not_absent_reader]] [[feedback_repair_scoped_to_named_cause]]

**What was paid:** §IV is retitled *"and what you are holding is not all of it"* and now states the
true position — the four documents are **not in this volume and not publicly deposited**, naming a
private repository would be an address you cannot use, and until the line carries a DOI the reader
should treat every *"the register says"* as **an authors' assertion they cannot presently check** and
grade it accordingly. The section also carries the finding's provenance: found cold, by an outsider,
using the book's own `VIII.3` standard, and the only independent finding of the first three reads.

★ **What is owed, and it is one action:** deposit the apparatus and paste the DOI. The bundle is
built and waiting at **`work/truth-and-consequences-apparatus.zip`** (561,390 bytes — claims
register, instruments, scaffold, architecture, revision queue, lexicon, the ground, plus a README
that says what each is and that they are working documents). Deposit it on Zenodo alongside the
PhilArchive upload, then replace the closing sentence of `C.1` §IV with the deposited address. ⚠
**Until that happens this row stays OPEN**, because a section that honestly reports an unredeemed
promise is a better book and is still an unredeemed promise. **TRIGGER: the PhilArchive upload
itself — same sitting, not after.** small, blocked on one deposit.

**CORRECTION — R-211's denominator.** The row reads *"Three reads, **8,749 words**"*. That is the
**fast three only**; the day's total is **five reads, ~21,678 words**. The ratio it computes (13 of
14 traceable to self-report, 1 independent) is correct **for the three** and must be cited with that
scope — the two ghost audits produced R-213 through R-233 in the consolidation table, including
R-214, which is the finding the row's thesis says a briefed reader does not produce. **The row's
conclusion survives and its evidence base was understated:** briefing manufactures agreement in
