...
|---|--------|-------|--------------|---------|------|
| **R-237** | — (opened Day 201, 2026-08-20, by a citation-lineage sweep, not a reading pass) | **VI.6**, and the METHOD reaches every chapter that cites a paper | **The two on-direction experiments have to be reconciled, or the disagreement has to be owned in the body rather than only in a note.** VI.6's turn rests on Gauthier et al. (Nature, Feb 2026): switching the algorithm **on** moves policy opinion +0.11–0.12 SD. **Liu et al., PNAS, 18 Feb 2025 — with Andrew Guess, the author of [^3], among the twelve —** ran the on-direction a year earlier across four experiments and ~9,000 participants and reports *limited causal effects on policy attitudes*. ✅ **PAID IN PART Day 201:** the chapter's claim about a set (*"nobody treated as a limitation"*) is repaired to *"rarely"*, the paper is cited at [^8] with its opposite verdict quoted, and the candidate reconciliation (PNAS has **no persistent social graph**, so the follow-acquisition mechanism VI.6 actually rests on cannot occur there) is stated **as a hypothesis**. **STILL OWED: neither full text has been read**, and the reconciliation is exactly the kind of interior claim the chapter's own grade note says it does not rely on. | ★ **THE FIRST TIME ANY CHAPTER RESTS WEIGHT ON THE ON/OFF ASYMMETRY AGAIN** — VI.6 is the only holder today, so this is a live gate, not a someday. **AND, separately and sooner: before upload, run the lineage query against [^4] and [^8] once more.** Both are ≤19 months old in a fast literature; R-9's trigger already names this exact hazard (*"gets HARDER to pay the longer it waits, because the literature moves"*) and R-9 was written about a different chapter. Two rows naming one mechanism is the pattern. | small (as repaired) / medium (full texts) |
| **R-236b** | — (Day 201, same sweep) | **apparatus, whole book** | **The lineage check is not in `tools/` and it found a real defect in one pass.** What was run by hand: extract each chapter's DOIs → OpenAlex → *later works by the same authors on the same primary topic* → subtract what the chapter already cites → read the residue. Over 8 documents it returned 155 raw candidates and **2** real ones, so it is **a reading list, not a gauge** — precision ~1.3%, and an alarm at that precision is furniture. ⚠ **Do not build it as a red light on a board.** Build it as a writing-time list, run once per chapter, output read by a person. | **Any new chapter that cites a paper published in the last five years, before its apparatus is graded.** ⚠ The residue is dominated by author productivity — a prolific author generates candidates whether or not they revised the claim; the filter that works is *"is this the same question asked again"* and it is a judgement, not a threshold. | medium |
