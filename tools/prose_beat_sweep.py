#!/usr/bin/env python3
"""prose_beat_sweep.py â€” ruling 33's instrument. Truth and Consequences, Day 187.
...
written when the scaffold was 68 chapters of plan and nothing was drafted (it is 67, and 43 are
drafted as of Day 189 — R-27), and on that corpus
it is complete. It is no longer that corpus. Fourteen chapters are PROSE now, and a beat in
`06` is not what those chapters contain â€” it is what they were once going to contain.
...
planned â€” *"there is no leavingâ€¦ there is no wall"* Â· *"nothing is under repair"* â€” and the
...
â˜…â˜… THE STANDING LESSON, and it is why this file exists rather than a patch to that one:
...
**has this move already been PERFORMED on the page?**
  ...
  ** SPENT **   the beat's distinctive vocabulary is already in a shipped paragraph, or a
  ...
  ?? TRACE      over the soft bar. Usually legitimate â€” Book II defines what Book III runs â€”
...
carries four-fifths of the words alone. The right question is CONTAINMENT â€” *what fraction of
what this beat plans to say is already on the page?* â€” and containment is exactly the metric
...
repeats I.4 out loud and says so â€” *"has already been said once, in a book that was not
permitted to argue"* â€” and that is the book working. So a hit is a question, never a verdict.
The exemption table is `beat_sweep`'s, imported rather than copied: an adjudication lives in
ONE place or it rots in the second (ruling 31, ruling 36 â€” a rule stated in one place and
...
# They are cross-checked, and a disagreement prints â€” see `drafted()`.
PROSE_HEAD = re.compile(r"^##\s+((?:[IVX]+|C)\.\d+)\s+â€”")
...
# âš  This is an ADMISSION GATE, which is where a gauge's design case goes to die (the standing
# note, learned when beat_sweep's floor of 4 excluded the three-content-word beat its own
# docstring named as the case that mattered). So the short beats are not dropped silently â€”
...
def paragraphs(text):
    """[(n, joined)] â€” prose split at blank lines, each block joined so a hard wrap cannot
...
def drafted(read):
    ...
    `read` is a callable so the fixture path can pull the same files out of git â€” the
    ...
    for name, text in read():
        ...
        if head and fid and head != fid:
            ...
            # without telling anyone. Say it, then trust the heading â€” the file says what it is.
            print(f"  âš  {name}: filename says {fid}, heading says {head} â€” trusting the heading")
        cid = head or fid
        if not cid:
            print(f"  âš  {name}: no chapter id in heading or filename â€” NOT SWEPT")
            continue
...
def contain(beat_w, para_w):
    ...
    the interesting words in this book are the ordinary ones â€” ground, render, place, world,
    focus â€” and any rarity weighting deletes exactly the vocabulary a repeat is made of.
...
# at run time from the drafted corpus, not hand-listed, so it cannot rot â€” but the threshold
...
def sweep(scaffold_text, dr, focus=None, exempt=True):
    ...
    if not chs:
        print("!! no chapters parsed â€” the scaffold's heading format changed")
        return None
    hv, _ = house(dr)
...
def report(chs, dr, undrafted, hits, short, hv, exempt=True, quiet=False):
    ...
    for rank, c, shared, cid, kind, text, pcid, ln, para in hits:
        ...
        if note:
            exempted += 1
            if not quiet:
                print(f"  --    {pcid} prose ~ {cid} {kind}   {c:.2f}  EXEMPT â€” {note[:88]}â€¦")
            continue
        if rank >= HARD:
            spent += 1
            print(f"  ** SPENT **   {cid} ({kind}) is already performed in {pcid}"
                  f"   containment {c:.2f}"
                  + (f" Â· shared {NGRAM}-gram: \"{' '.join(next(iter(shared)))}\"" if shared else ""))
        else:
            traces += 1
    ...
    print()
    print(f"  {len(dr)} drafted chapter(s) Â· {sum(len(p) for p in dr.values())} paragraphs Â· "
          f"{len(undrafted)} undrafted Â· {spent} spent Â· {traces} trace(s) Â· {exempted} exempt")
    print(f"  {short} beat(s) below the {MIN_BEAT}-distinct-word floor â€” NOT dropped: reachable")
    print(f"  by the {NGRAM}-gram discriminator only, which is what a beat that IS a phrase needs.")
    print(f"  house vocabulary discounted, measured at df â‰¥ {HOUSE_DF:.2f}: {len(hv)} word(s)"
          + (f" â€” {', '.join(sorted(hv)[:12])}â€¦" if hv else ""))
    if spent:
        print()
        print("  SPENT is a QUESTION, not a verdict: the chapter may still be RIGHT to return")
        print("  to the move â€” II.4 repeats I.4 deliberately and SAYS SO on the page, and that")
        print("  is the strongest seam in the work. What it may not do is return SILENTLY.")
        print("  Answer it in `06` â€” cut the beat, or flag the reprise out loud â€” and add the")
        print("  PAIR to beat_sweep.EXEMPT with the ruling. Do not raise the threshold.")
    return spent


# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# THE SEMANTIC ARM â€” and it exists because the lexical arm above FAILED its own
...
# sitting on top of I.6's shipped *"There is no leavingâ€¦ There is no wallâ€¦ Nothing is under
# repair."* Those two sentences perform the same act and **share not one content word**. Every
# discriminator above â€” containment, 5-gram, trigram â€” is a gauge whose unit is the WORD, and
# the standing note already says what that costs:
#
#   â˜…â˜… A DUPLICATE OF **FUNCTION** IS INVISIBLE TO A GAUGE WHOSE UNIT IS THE WORD (ruling 32).
#
...
# cannot see.
#
# âš âš  AND THE RAW COSINE IS NOT THE MEASUREMENT â€” NOR IS A Z-SCORE. Both were built here and
# both failed, and the failures are worth more than the fix.
#
# bge-m3 is anisotropic: two unrelated texts sit near 0.50 by the model's own geometry â€”
...
# arithmetic**. Every beat has a best match, so `z(best) â‰¥ 3` selects nearly every beat. The
# first run returned 143 hits, which is the same thing as returning none.
#
# â˜…â˜… A NULL FOR THE MEAN IS NOT A NULL FOR THE MAXIMUM. The statistic under test was an
...
# own beat â€” *"the Return defined against escape, salvation, and exit"* â€” I.6's *"Not by leaving
# it. There is no leavingâ€¦"* ranks **3rd of 390 paragraphs**, at cosine 0.489, under a top match
...
#
# Mutual rank was tried next â€” reciprocal nearest neighbours, which is offset-immune because it
...
#
# â˜…â˜…â˜… SO THE ARM DOES NOT RETURN A VERDICT, AND THAT IS THE FINDING RATHER THAN A RETREAT.
#
...
# threshold that made the calibration pass would have been fitted to one known answer â€” a
...
# READING BRIEF â€” for each beat of the chapter about to be written, the handful of already-
...
def brief(scaffold_text, dr, focus, n=BRIEF_N, show=True):
    ...
    Deliberately NOT a detector â€” see the block comment above. It ranks and prints; a person
    reads. A `None` return means the arm did not run, and that prints loudly rather than
    passing as a clean sweep."""
    fn, note = embedder()
    if fn is None:
        print(f"  âš  READING BRIEF NOT RUN â€” no embedder ({note}).")
    ...
    if not plan or not prose:
        print("  âš  reading brief: nothing to compare.")
        return None
    if show:
        print(f"  {note} Â· {len(plan)} beat(s) Ã— {len(prose)} shipped paragraph(s)"
              f" Â· top {n} each, RANKED, no verdict")
...
def brief_report(rows, exempt=True, quiet=False):
    ...
    print("  built this arm showed no statistic separates a true repeat from the floor â€”")
...
DRAFTED_MARK = re.compile(r"^### ((?:[IVX]+|C)\.\d+)\b.*?(âœ…\s*DRAFTED)?$")
...
# while `06` carried a âœ… for each of them. Books I and II put the tick INLINE
# (`### II.1 â€” THE GROUND âœ… DRAFTED â€” 2,282 words`); Book III put it on its own line
...
#   The fix reads the chapter's whole BLOCK â€” heading to next `###` â€” and accepts a line that
...
# to open its own line with a âœ… to count, which is a thing the format does not do.
BLOCK_MARK = re.compile(r"^âœ…\s*\*{0,2}DRAFTED", re.M)


def status(scaffold_text, dr):
    """`06`'s âœ… DRAFTED markers against what is actually in `book/`.

    Why this is a function and not ten more hand-typed ticks: a `âœ… DRAFTED` marker is a
    *"Last Verified"* stamp with a checkmark on it. Somebody asserts it once and it rots in
    silence while the thing it describes moves on â€” which is exactly what happened. The joint
    ...
    âš  AND THEN IT ROTTED ITSELF â€” ruling 81. See BLOCK_MARK above: this function read the
    ...
    for n, (i, cid) in enumerate(heads):
        ...
        if "DRAFTED" in lines[i] or BLOCK_MARK.search(block):
            ...
            missing.append(cid)
    print(f"DRAFT STATUS â€” `06` markers against book/\n")
    print(f"  {len(dr)} chapter(s) are prose on disk Â· {len(marked)} marked âœ… in the scaffold")
    if missing:
        print(f"  !! {len(missing)} DRAFTED AND UNMARKED â€” the scaffold under-reports the work:")
    ...
    if phantom:
        ...
        print(f"  !! {len(phantom)} MARKED âœ… WITH NO FILE IN book/ â€” the scaffold claims prose")
...
def selftest():
    ...
    rc = beat_selftest()
    doc = ("# BOOK I â€” THE STILL\n\n## I.9 â€” A TEST\n\nNot by leaving it. There is no\n"
           "leaving â€” leaving wants a direction.\n\n---\n\nNothing is under repair.\n")
    ps = paragraphs(doc)
    joined = " | ".join(p for _, p in ps)
    ok = ("There is no leaving â€” leaving wants a direction." in joined
          and len(ps) == 2 and "A TEST" not in joined)
    print("  prose self-test:", "PASS â€” needles survive a hard wrap, headings excluded"
          if ok else f"** FAIL ** â€” got {ps!r}")
...
def main():
    ...
    ap.add_argument("--brief", action="store_true",
                    help="with --chapter: the pre-draft reading brief â€” the shipped paragraphs "
                         "nearest each beat, ranked, for a person to read. No verdict.")
    ap.add_argument("--status", action="store_true",
                    help="`06`'s âœ… DRAFTED markers against what is actually in book/")
    ...
    if args.fixture:
        ...
        if raw.returncode != 0:
            ...
            return 2
        print(f"CALIBRATION â€” tree at {args.fixture}, exemptions OFF\n")
        ...
        print()
        print("READING BRIEF â€” II.8, as it stood before it was drafted")
        ...
        # nothing behind it â€” the whole failure this project keeps finding. The page a person
        ...
        # the measurement supports. I.6's *"Not by leaving it. There is no leavingâ€¦"* must be
        ...
        if found:
            ...
            print(f"  CALIBRATION PASS â€” I.6's prose appears on II.8's pre-draft brief, "
            ...
            return 0
        print("  ** CALIBRATION FAIL ** â€” I.6 is not on II.8's brief at all.")
        print("  This instrument exists to put exactly that paragraph in front of a reader.")
        return 2

    print(f"PROSEâ†’BEAT SWEEP â€” book/ against {SCAFFOLD.name}"
          + (f" Â· chapter {args.chapter}" if args.chapter else "") + "\n")
    ...
    if args.brief:
        if not args.chapter:
            ...
            return 2
        print(f"\nREADING BRIEF â€” {args.chapter}, before drafting")
        brief_report(brief(text, dr, focus=args.chapter), quiet=args.quiet)
    else:
        print("\n  âš  THE FUNCTION ARM DID NOT RUN. The sweep above is word-unit only and is")
        print("  blind to a repeat that shares no vocabulary â€” ruling 33's own case shared")
