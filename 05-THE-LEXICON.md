...
|---|---|---|---|
| **the Ground** | **KEEP** | "base reality" = Bostrom's server room, where the real people are | It is not elsewhere and not more real than here. It is what *here* is made of. *(Ruling 6; and it is not a coinage — `Perspective` already says "the ground.")* |
...
| **the Narrowing** | **KEEP** | none serious | The act; the Perspective is its result. |
...
| **Coherence** | **KEEP** | "coherent" as a synonym for "makes sense" | Structural consistency across scales *and* the felt alignment of a stream with its own trajectory — the claim is that these are one thing. |
...
| **the player** | **CAPTURE** | A player implies someone outside holding a controller. There is nobody outside. **The player is a character the whole game contains.** *(Watts.)* |
...
| **the map** | **RETIRE** | ⚠ **Correction to `01-THE-GROUND.md`.** Its table says "the map is pre-rendered and complete." Two faults: *map* imports representation-*of* (the Ground represents nothing), and Korzybski's *the map is not the territory* is load-bearing in Book VI, where "map" must mean the model and not the world. Using it for the Ground collides with our own ancestor. Replaced by **the whole game** — §5. |
...
| **save / respawn / level / quest / sandbox** | **UNRULED — do not use until ruled** | Each smuggles a claim about death, progress, or authorship that Book VII has not yet made. A metaphor that runs ahead of the argument is how the game frame would start doing our thinking for us. |
